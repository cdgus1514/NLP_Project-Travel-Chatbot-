...
def embeded(data):
    ...
    encode_raw = okt.morphs(data)
    encode_raw = list(map(lambda x: encode_raw[x] if x < len(encode_raw) else "#", range(vector_size)))
...
def create_graph(train=True):
    ...
    for i, filter_size in enumerate(filter_sizes):
        with tf.name_scope("conv-maxpool-%s" % filter_size):
            ...
            # Debug6
            print("[Debug6]intent.classifier create_graph filter_shape", filter_shape)
...
def get_intent(text):
    prediction = predict(np.array(embeded(text)).flatten)
