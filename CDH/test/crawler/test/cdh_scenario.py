...
from crawler_travel import recommand_travelCity
from search_attraction import recommand_attraction
...
def attraction(named_entity):
    ...
    for k in zip(keyword_group, entity_group):
        if 'LOCATION' in k[1]:
            location.append(k[0])
        elif 'TRAVEL' in k[1] or 'PURPOSE' in k[1]:
            travel.append(k[0])
    ...
    print("[DEBUG3-3]scenario dust (location) >> ", location, end="\n")
    print("[DEBUG3-3]scenario dust (travel) >> ", travel, end="\n")

    return recommand_attraction(' '.join(location), ' '.join(travel))